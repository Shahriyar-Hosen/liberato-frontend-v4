...
import { ICategory, ILocation, QuestionAndAnswer } from 'types/admin';
...
export default function EditForm({
  ...
  const [category, setCategory] = useState<string>(location.category.id);
  const [questionsAndAnswer, setQuestionsAndAnswer] = useState<
    QuestionAndAnswer[] | []
  >(location.questionsAndAnswers);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [changePicture, setChangePicture] = useState<boolean>(false);
  const [success, setSuccess] = useState<ModalType>(initialModalState);
  const [error, setError] = useState<ModalType>(initialModalState);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      if (!location.images && !selectedFile) {
        setError({
          state: true,
          message: 'You need to add an image',
      ...
      if (questionsAndAnswer.length > 0) {
        let qaString = '';
        if (category === location.category.id) {
          questionsAndAnswer.forEach((q: QuestionAndAnswer) => {
            qaString += `${q.answerId}:${q.answer},`;
          });
      ...
      formData.append('city', location.city.name);
      ...
      const config = {
        ...
        url: `${apiUrl}/locations/${location.id}?_method=PUT`,
        headers: {
  ...
  useEffect(() => {
    ...
    if (questions && category !== location.category.id) {
  ...
  if (success.state) {
    return (
      <SuccessModal
        ...
        isOpen={success.state}
        close={() => setSuccess({ state: false, message: '', title: '' })}
      />
    );
  }
  ...
  return (
    <div className='text-center'>
      <h1 className='text-3xl font-bold text-indigo-500'>Edit location</h1>
      ...
      <form className='mt-8' onSubmit={formik.handleSubmit}>
            ...
            <div className='w-full'>
              ...
              <div className='mt-1'>
                <input
                  ...
                  name='street'
                  id='street'
                  defaultValue={formik.values.street.split(',')[0]}
                  onChange={formik.handleChange}
            ...
            <div className='mx-auto max-w-2xl lg:mx-0'>
                ...
                >
                  {categories.map((cat: ICategory) => (
                    <option
                      value={cat.id}
                      key={cat.id}
                      selected={cat.name === location.category.name}
                    >
                      {cat.name}
                    </option>
                  ))}
              ...
              <div className='mt-4'>
                ...
                <select
                  ...
                  disabled
                >
                  <option key={location.city.id}>{location.city.name}</option>
                </select>
              </div>

              <div className='mt-2 text-sm text-red-700'>
                <p>You can not change city!</p>
              </div>
            </div>
        ...
        {location.images?.length && !changePicture && (
          <div className='mt-5 sm:mt-6'>
            <Image
              ...
              alt='preview'
              className='mx-auto max-h-80 max-w-4xl object-contain'
            />
          </div>
        )}
        {changePicture && !selectedFile && (
        ...
        <div className='mt-10 flex justify-center'>
          {!changePicture && (
            <button
              ...
              className='mr-4 inline-flex items-center rounded-full border border-transparent bg-indigo-600 px-6 py-3 text-base font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50'
            >
              Change image
            </button>
          )}
